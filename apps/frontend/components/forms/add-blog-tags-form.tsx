...
import { adminAddBlogTag } from "@/lib/actions/blog-tags-actions";

const AddBlogTagsForm = () => {
  const router = useRouter();
  const [isSubmitting, startTransition] = React.useTransition();

  const form = useForm<BlogTagSchemaType>({
  ...
  const onSubmit = async (values: BlogTagSchemaType) => {
    startTransition(async () => {
      try {
        const response = await adminAddBlogTag(values);
        if (!response.success) {
...
export default AddBlogTagsForm;
